	...
	methods: {
		...
		async generateQRCode() {
			...
			try {
				...
				console.log('[前端] 响应类型检查:', {
					hasCode: !!response?.code,
					hasData: !!response?.data,
					hasQrToken: !!response?.qrToken,
					hasDataQrToken: !!response?.data?.qrToken,
					responseKeys: response ? Object.keys(response) : []
				})
				
				// 兼容两种返回格式：1. { code: '200', data: { qrToken, ... } }  2. { qrToken, ... } 直接返回
				let qrToken = null
				...
				} else if (response && response.qrToken) {
					// 格式2：直接返回QrCodeResponse对象
					qrToken = response.qrToken
					refreshInterval = response.refreshInterval || 60
				...
				if (qrToken) {
					...
					console.log('[前端] 二维码图片URL已生成:', this.qrCodeUrl)
