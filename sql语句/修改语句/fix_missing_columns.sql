...
-- 检查并添加 reserved_for_patient_id 字段（如果不存在）
SET @col_exists = (
    SELECT COUNT(*) 
    FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE()
...
-- 检查并添加 slot_application_id 字段（如果不存在）
SET @col_exists = (
    SELECT COUNT(*) 
    FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE()
...
-- 检查并添加索引 idx_reserved_patient（如果不存在）
SET @idx_exists = (
    SELECT COUNT(*) 
...
-- 检查并添加索引 idx_slot_application（如果不存在）
SET @idx_exists = (
    SELECT COUNT(*) 
    FROM INFORMATION_SCHEMA.STATISTICS 
    WHERE TABLE_SCHEMA = DATABASE()
...
-- 2. 修复 appointments 表（如果需要）
-- 检查并添加 payment_deadline 字段（如果不存在）
SET @col_exists = (
    SELECT COUNT(*) 
    FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE()
...
-- =====================================================
-- 执行说明：
-- 1. 此脚本会自动检查字段是否存在，如果已存在会跳过并显示提示
