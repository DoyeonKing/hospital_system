...
2. schedules表：新增 location 字段，用于记录具体的就诊地点
3. 新增 symptom_department_mapping表：支持基于症状关键词的AI智能挂号推荐
4. 新增 medical_guidelines表：用于管理校医院的各项就医规范和流程
5. 组织架构调整：使用 parent_departments 和 departments 两级科室结构
6. 新增地图导航模型：包含 map_nodes 和 map_edges 表，支持院内导航功能
