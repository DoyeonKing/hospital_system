...
import com.example.springboot.dto.recommendation.DepartmentRecommendationRequest;
import com.example.springboot.dto.recommendation.DepartmentRecommendationDTO;
import com.example.springboot.dto.recommendation.DoctorRecommendationByDepartmentRequest;
import com.example.springboot.dto.recommendation.DoctorRecommendationWithScheduleDTO;
...
public ResponseEntity<?> recommendDepartments(@RequestBody DepartmentRecommendationRequest request) {
    try {
        ...
        logger.error("科室推荐失败：事务回滚异常", e);
        return new ResponseEntity<>("推荐失败：系统内部错误，请稍后重试",
...
public ResponseEntity<?> recommendDoctorsByDepartment(
    ...
    try {
        ...
        List<NLPService.DoctorRecommendationWithSchedule> recommendations =
                nlpService.recommendDoctorsByDepartment(
        ...
        logger.error("按科室推荐医生失败：事务回滚异常", e);
        return new ResponseEntity<>("推荐失败：系统内部错误，请稍后重试",
                HttpStatus.INTERNAL_SERVER_ERROR);
    } catch (Exception e) {
...
private DoctorRecommendationWithScheduleDTO convertWithScheduleToDTO(
        NLPService.DoctorRecommendationWithSchedule rec) {
