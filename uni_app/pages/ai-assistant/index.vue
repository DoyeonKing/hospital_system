			...
			async createAppointment(doctor, schedule) {
				...
				try {
					...
					if (response && response.code === '200') {
						...
						uni.navigateTo({
							url: `/pages/payment/payment?scheduleId=${schedule.scheduleId}&appointmentId=${appointmentId}&fee=${fee}&doctorName=${encodeURIComponent(doctor.doctorName)}&doctorTitle=${encodeURIComponent(doctor.title || '')}&departmentName=${encodeURIComponent(doctor.departmentName)}&scheduleDate=${schedule.scheduleDate}&slotName=${encodeURIComponent(slotName)}&location=${encodeURIComponent(schedule.location || '')}`
						})
					} else {
						// 处理各种错误情况
