  ...
  "schedules": [
    {
      ...
      "slotId": 1,
      "slotName": "上午 08:00-09:00",
      "locationId": 10,
      "locationName": "门诊楼201室",
      "totalSlots": 20,
      "bookedSlots": 0,
      "fee": 5.00,
      "status": "available",
...
| 序号 | 接口路径 | 方法 | 描述 | 权限 |
|------|----------|------|------|------|
| 1 | `/api/schedules/auto-generate` | POST | 自动生成排班 | ADMIN |
| 2 | `/api/schedules/auto-generate/rules` | GET | 获取默认规则 | ADMIN |
| 3 | `/api/schedules/validate` | POST | 验证排班冲突 | ADMIN |
...
该方案基于你现有的数据库结构和技术栈设计，无需修改现有代码，采用扩展方式实现，风险可控。
